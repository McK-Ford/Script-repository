...
MB231_ordered <- copy(pI_tab)
setorder(MB231_ordered, cols = - "pI.MB231", na.last=TRUE)
SUM159_ordered <- copy(pI_tab)
setorder(SUM159_ordered, cols = - "pI.SUM159", na.last=TRUE)
```
These aren't actually setting the order even though the first one is. Why, weird thing with data.table? Oh right it's not taking a copy so it won't modify properly. Ugh. Okay, modifying these commands from "MCF10A_ordered <- pI_tab" to "MCF10A_ordered = copy(pI_tab)" and that should work.
```{r}
