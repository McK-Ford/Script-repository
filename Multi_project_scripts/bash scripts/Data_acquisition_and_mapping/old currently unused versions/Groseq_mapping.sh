...
awk '{print $2}' < samples.txt > samples2.txt
sampleFiles=$(<samples2.txt)
...
##THIS SECTION HANDLES SE LIBRARIES
	echo "mapping ${sample} to human genome"
               bowtie2 -t -x ${reference} \
               	-U ${sample}.fastq.gz -S ${sample}.sam \
...
##maxins is the maximum fragment length. -no-discordant is nothing aligns singly, and no mixed too.
        ...
        echo "filtering ${sample} with mapping quality < ${mappingQuality}, remove mitochrondria"
        sed '/chrM/d;/chrY/d;/random/d;/alt/d;/chrUn/d' < ${sample}.sam > ${sample}.filtered.sam #remove mitochondria, Y chromosome, etc.
