...
test_p$uniq_ID = test_p$V4
test_m$uniq_ID = test_m$V4
#Get the processed values with their metaplots
p_with_verified = merge(test_p, plus_vals, by="uniq_ID", all=TRUE)
m_with_verified = merge(test_m, minus_vals, by="uniq_ID", all=TRUE)
...
####################
col_trimmed_p = has_signal_p %>%
  select(-c(
    V4, V6, V11, chrom, V13, V14, V15, V16, V20,
    V21, V22, V23, V25, V27, V30, V31
    ))
col_trimmed_p_silent = silent_p %>%
  select(-c(
    V4, V6,  V11, chrom, V13, V14, V15, V16, V20,
    V21, V22, V23, V25, V27, V30, V31
    ))
col_trimmed_m = has_signal_m %>%
  select(-c(
    V4, V6, V11, chrom, V13, V14, V15, V16, V20,
    V21, V22, V23, V25, V27, V30, V31
    ))
col_trimmed_m_silent = silent_m %>%
  select(-c(
    V4, V6, V11, chrom, V13, V14, V15, V16, V20,
    V21, V22, V23, V25, V27, V30, V31
    ))
...
save(col_trimmed_p, col_trimmed_m, col_trimmed_p_silent,
     col_trimmed_m_silent, file="gettingTruTSSes.RData")
