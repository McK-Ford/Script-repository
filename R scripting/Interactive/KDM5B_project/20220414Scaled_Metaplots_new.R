...
notstranded_bam_list = list(paste0(
  ...
  "/Users/kayle/Box/Vertinolab/McKayla Ford/SRR15343279.dedup.bam"
)
...
upstream_flat = upstream[,.(H2AZ=mean(Score), H3K4me3=mean(V2),
                            KDM5B=mean(V3), CTCF=mean(V4)),by=Bins]
...
CGI_scores_melt = CGI_scores %>% 
  select(-c(1)) %>% pivot_longer(cols=c(H2AZ, H3K4me3, CTCF, KDM5B))
...
write.table(CGI_scores, "scaled_factors.txt")