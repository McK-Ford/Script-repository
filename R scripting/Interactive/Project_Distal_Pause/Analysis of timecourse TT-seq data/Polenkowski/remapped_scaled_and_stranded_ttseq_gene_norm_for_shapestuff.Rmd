---
title: "Analysis of stranded/scaled timecourse TTseq w/ signal gene-normalized (to signal at TSS)"
...
Okay, this time I want to correct my genes so highly expressed genes don't skew everything, but without doing trimmed mean because I'm not entirely certain that's the best way to represent the data either. So I want to find maximum value per gene, and then divide each column by that to get (percent of gene maxima). I also don't want to take the max just by condition though, because that would eliminate height differences between conditions.

So, here's how I do this. Get max by row (28-429), then group by gene ID. Instead of summarizing output, make new column based on the maximum by group, then replace columns 28-429 with newcol/28-429. 

helpful stackoverflow source(https://stackoverflow.com/questions/28486654/row-maximum-in-data-table)
```{r}
tab[, max1:= do.call(pmax, .SD), , .SDcols = c(28:427)]
```
Okay now get max by group. I might have been able to do these together, but as it is, the syntax is already quite complicated.
```{r}
tab[, maxScore:= max(max1), uniq_id]
```
if max is 0 should probably remove gene
then divide by max (maxScore bc r gets confused if you name it max)
```{r}
tab = tab[maxScore != 0]
...
```{r}
geom <- len_quant_ds |> 
  filter( loc == "CGI5" & senseanti == "antisense" & simple_skew_class == "Prox" & condition == "ctrl") |>
  ggplot( aes( x = Bins, y = mean_score, color = perc )) +
      geom_line() +
      mytheme +
...
```{r}
geom <- len_fixed_5_us |> 
  filter( loc == "TSS" & senseanti == "antisense" & simple_skew_class == "Dist" & condition == "ctrl") |>
  ggplot( aes( x = Bins, y = mean_score, color = perc )) +
      geom_line() +
      mytheme +
