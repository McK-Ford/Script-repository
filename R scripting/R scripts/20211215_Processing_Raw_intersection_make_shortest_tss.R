...
shortest_tss_in_cgi <- plus_TSS_intersect_only %>% group_by(cgi_s) %>% mutate(tss=max(gene_s)) %>% filter(tss==gene_s) #
#8702 genes
###################################
...
minus_TSS_intersect_only <- minus_strand_only %>%
  filter(cgi_e>gene_e)
shortest_tss_in_cgi_m <- minus_TSS_intersect_only %>% group_by(cgi_s) %>% mutate(tss=min(gene_e)) %>% filter(tss==gene_e) #drops it to 8457
