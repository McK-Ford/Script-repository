...
annotateGtf = function (gtfFile, outFile = c(NA)) {
	...
	#summarize by id
	genes = gtf[, .(chr = unique(seqnames), start = min(start), end = max(end), strand = unique(strand), symbol = unique(gene_name), gene_biotype = unique(gene_biotype), source = paste0(unique(source), collapse = "|")), by = gene_id]

	#summarize protein ids that are not NA
	pids = gtf[!is.na(protein_id), .(protein_id = paste0(unique(protein_id), collapse = "|")), by = gene_id]

	#summarize coding start and stop
	cds = gtf[type == "CDS", .(cds.start = min(start), cds.end = max(end)), by = gene_id]

	#merge
	genes = merge(genes, cds, by = "gene_id", all.x = T)
	genes = merge(genes, pids, by = "gene_id", all.x = T)

	if (!is.na(outFile)) fwrite(genes, outFile, sep = "\t", row.names = F)
